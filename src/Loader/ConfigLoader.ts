...
export async function loadFromJSONLike(json: JSONLike): Promise<Config> {
  ...
  // char*
  await Promise.all(char.map((_char, id)=>{
...
export async function loadCharConfig(char: JSONLike): Promise<Char> {
  const that = new Char();
